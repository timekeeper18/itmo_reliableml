...
- Бизнес-цель: разработать сервис по обнаружению видимого кариеса на фотографиях зубов (челюсти), сделанных с помощью камеры мобильного телефона. Добиться полноты (Recall) обнаружения кариеса не менее <span style="color:red">79%</span>. 
...
- Логика работы ML: Модель работает как веб-сервис. По адресу сайта системы есть возможность загрузить фотографию. В ответ система выдает полученное изображение с выделением зубов, где обнаружен кариес, а так же рекомендацию консультации стоматолога. 
- Бизнес-требования: модель должна работать с фотографиями, сделанными на камеру современного (2-3 года) смартфона. **подумать над ограничениями на смартфоны**
- Что будем считать успехом итерации? Успехом итерации будем считать работающий MVP с возможностью загрузить фотографию челюсти (нижней/верхней) и получить ответ об обнаружении на снимке кариеса <span style="color:red">с необходимой полнотой обнаружения</span>.
...
- Возможные пути развития проекта. Оценка процента видимого поражения зуба, ведение истории и наблюдение состояния зубов  в динамике, обнаружение других заболеваний полости рта.
- Среда: разработка на личных компьютерах, развертывание в облачном сервере.
- Стэк-технологий: Python, FastAPI, pytorch
...
- Что не будет закрыто: **дополнить**
...
В качестве данных -  изображения (фотографии зубов), одна фотография - нижняя или верхняя челюсть, сфоторграфированная под углом максимально близким к 90 градусам, охватывающая все зубы на этой челюсти.
### 2. Методология

#### 2.1. Постановка задачи  

- Что делаем с технической точки зрения: V1.0 - сегментация и классификация
...
**Этап 1 - поиск и анализ датасетов, подготовка данных.**

| Название датасета         | Ссылка на датасет                                                                              | Размер датасета | Описание                                    | Подходит? |
|---------------------------|------------------------------------------------------------------------------------------------|-----------------|---------------------------------------------|-----------|
| dental-app                | https://universe.roboflow.com/dental-app/dental-app                                            | 32/167          | Отличие кариеса от других болезней          |           |
| occlusal-caries-detection | https://universe.roboflow.com/predict-caries-1/occlusal-caries-detection-olf03/                | 56/61           | Есть в плохом качестве                      |           |
| dental-project-bvtip      | https://universe.roboflow.com/dental-project-fcai/dental-project-bvtip                         | 463/1691        | без меток 0                                 |           |
| dentasis                  | https://universe.roboflow.com/dentasis/dentasis                                                | 517/1129        | Кариес 2 вида  без меток 612                |           |
| mega-throat-data          | https://universe.roboflow.com/vitekthebest/mega-thoart-data/                                   | 544/1864        | без меток 0 есть метки зубов (детские фото) |           |
| caries-scjtb              | https://universe.roboflow.com/vitekthebest/caries-scjtb                                        | 296/410         | без меток 4 есть метки зубов (детские фото) |           |
| deenfermedades-dental     | https://universe.roboflow.com/proyecto-odontologia/proyecto-deteccion-de-enfermedades-dentales | 76/204          | без меток 4 (yolo8 нет)                     |           |
| xd_2                      | https://universe.roboflow.com/111-j3wer/xd_2                                                   | 448/900         | без меток 452                               |           |
| toothcaries               | https://universe.roboflow.com/odontoapp/toothcaries                                            | 201/201         | без меток 0 (yolo8 нет)                     |           |
| yolo-convertion-fbsju     | https://universe.roboflow.com/yolo-lgmeg/yolo-conversion-fbsju                                 | 5               | только кариес                               |           |
| 111-j3wer                 | https://universe.roboflow.com/111-j3wer/our                                                    | 56              | только кариес (детские фото)                |           |
 
 Данные плохо размечены, много неподходящих фото. Необходимо отобрать фотографии на которых хорошо видны зубы и корректная разметка. Для перехода на следующий этап необходимо собрать датасет не менее:
 1) для бейзлайна - 50/200
 2) для MVP - 450/1000 примеров.

**Этап 2 - разработка пайплайна загрузки и аугментации изображений**
**Этап 3 - обучение и сравнение моделей классификации**
